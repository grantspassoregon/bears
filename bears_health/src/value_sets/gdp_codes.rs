...
#[tracing::instrument]
fn get_gdp_keys<'a, P: AsRef<std::path::Path> + std::fmt::Debug>(
    path: P,
    dataset: Dataset,
...
#[tracing::instrument]
pub fn gdp_keys(dataset: Dataset) -> Result<(), BeaErr> {
    let path = bea_data()?;
    let data = get_gdp_keys(&path, dataset)?;
    let target = path.join(format!("{dataset}_Frequency_Params.json"));
    write_json(data.frequencies(), target)?;
    let target = path.join(format!("{dataset}_Industry_Params.json"));
    write_json(data.industries(), target)?;
    let target = path.join(format!("{dataset}_TableID_Params.json"));
    write_json(data.tables(), target)?;
    let target = path.join(format!("{dataset}_Year_Params.json"));
    write_json(data.years(), target)?;
    Ok(())
}
